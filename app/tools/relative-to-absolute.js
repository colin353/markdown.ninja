/*
  ...
  are run on the root domain but reference data on the subdomains.

  I copied this from a StackOverflow post and made some changes so
  it would work on my system. You can find the original code here:
...
function rel_to_abs(url){
  ...
  if(/^(https?|file|ftps?|mailto|javascript|data:image\/[^;]{2,9};):/i.test(url))
    ...
    var target_url = location.protocol + "//" + api.user.domain + "." + api.BASE_DOMAIN + "/index.md";
    var base_url = target_url.match(/^(.+)\/?(?:#.+)?$/)[0]+"/";
    if(url.substring(0,2) == "//")
        return location.protocol + url;
    else if(url.charAt(0) == "/")
        return location.protocol + "//" + api.user.domain + "." + api.BASE_DOMAIN + url;
...
module.exports = replace_all_rel_by_abs;
